      ...
      var childrenFunc = function(doc) {
        ...
        for(var i=0; i< doc.childNodes.length; i++) {
          ...
          if (response.nodeType == 1) {
            ...
            var resourcetype;
            for(var j=0; j < prop.childNodes.length; j++) {
              if (prop.childNodes[j].nodeName.indexOf('resourcetype') >= 0) {
                resourcetype = prop.childNodes[j];
                break;
              }
            }
            if (resourcetype) {
              ...
              if(collection) {
                result[resultIndex++] = new fs.dir(href);
              } else {
                result[resultIndex++] = new fs.file(href);
              }
            }
          }
        }
